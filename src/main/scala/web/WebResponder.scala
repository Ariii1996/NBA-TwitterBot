...
class WebResponder() extends Responder {
  ...
  def respondNextGame(game: JsValue, _request: Request, team_name: String): Unit = {
    ...
    val htmlScreen = s"""
      ...
      <body>
        <div class="request">
          <h1 class="requestTitle"> Proximo partido de los $team_name es: </h1>
  ...
  def respondPlayerStats(stats: Seq[Any], _request: Request): Unit = {
    ...
    val htmlScreen = s"""
      ...
      <body>
      <div class="request">
          <h1 class="requestTitle"> Datos del jugador ${stats.head}: </h1>
  ...
  def respondError(_request: Request, message: String): Unit = {
    ...
    val htmlScreen = s"""
      ...
      <body>
        <div class="request">
          <h1 class="requestTitle"> $message </h1>
        </div>
      </body>
    """
    request.complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, htmlScreen))
  ...
  def respondInternalError(_request: Request): Unit = {
    ...
    val htmlScreen = """
      ...
      <body>
        <div class="request">
          <h1 class="requestTitle"> Lo sentimos, ocurrió un error en nuestros servidores</h1>
        </div>
      </body>
    """
    request.complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, htmlScreen))
